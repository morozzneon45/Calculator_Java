...
public class StringCalculator {
    public static void main(String[] args) throws Exception {
        ...
        int multiplier = dividerMultiplier(part);
    ...
    private static void Calculate(char oper, String[] part, int multiplier) {
        if (oper == '+') { //сочетание
    ...
    private static int dividerMultiplier(String[] part) throws Exception {
        int multiplier = 1; //множитель
        int divider = 1; //делитель
        try {
            multiplier = Integer.parseInt(part[1]);
            divider = Integer.parseInt(part[1]);
            if (multiplier < 1 || multiplier > 10 || divider < 1 || divider > 10) { //исключение, если число > 10
                throw new Exception();
            }
